...
class DataContainer:
    ...
    def __getitem__(self, idx):
        ...
        no_atoms_per_row = np.repeat(data["N"], data["N"])
        indptr = np.concatenate([[0], np.cumsum(no_atoms_per_row)])
        indices = np.concatenate([np.arange(0, n, dtype=np.int32) for n in no_atoms_per_row], axis=0)
        indices += np.repeat(np.repeat(np.concatenate([[0], data["N"][:-1]]), data["N"]), no_atoms_per_row)
        mat_data = np.ones(shape=(np.sum(data["N"] ** 2),))
        shape = (np.sum(data["N"]), np.sum(data["N"]))
        ind_matrix = sp.csr_array((mat_data, indices, indptr), shape=shape)

        atom_pairs = ind_matrix.nonzero()

        atom_pair_i = atom_pairs[0]
        atom_pair_j = atom_pairs[1]

        data["atom_pair_indices"] = np.concatenate([atom_pair_i, atom_pair_j]).T

        atom_pair_mol_id = np.repeat(np.arange(len(data["N"])), np.repeat(data["N"], data["N"]))

        data["atom_pair_mol_id"] = atom_pair_mol_id
