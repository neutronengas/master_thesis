...
class Trainer:
    def __init__(self, model, learning_rate=1e-3, warmp_steps=None,
        ...
        if warmp_steps is not None:
            self.learning_rate = LinearWarmupExponentialDecay(
                learning_rate, warmp_steps, decay_steps, decay_rate
            )
        else:
        ...
        if model.built:
            self.backup_vars = [tf.Variable(var, dtype=var.dtype, trainable=False) for var in self.model.trainable_weights]
        else:
    ...
    def train_on_batch(self, dataset_iter, metrics):
        ...
        with tf.GradientTape() as tape:
            ...
            mae = tf.reduce_mean(tf.abs(preds - targets), axis=None)
